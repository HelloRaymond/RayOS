...
xdata u8	TX1_Buffer[COM_TX1_Lenth];	//发送缓冲
xdata u8 	RX1_Buffer[COM_RX1_Lenth];	//接收缓冲
xdata u8	TX2_Buffer[COM_TX2_Lenth];	//发送缓冲
xdata u8 	RX2_Buffer[COM_RX2_Lenth];	//接收缓冲
...
/********************* UART1中断函数************************/
INTERRUPT(UART1_int, UART1_VECTOR)
{
	if(RI)
	{
		RI = 0;
