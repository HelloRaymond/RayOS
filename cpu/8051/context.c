...
extern idata ray_base_t OS_XStackBuffer[]; //The actual stack when the thread is running, all threads share this stack
extern ray_thread_t OS_ThreadHandlerIndex[];
extern ray_uint8_t OS_RunningThreadID;

SFR(SP_REG, 0x81);
idata ray_base_t OSStack[10];
...
INTERRUPT(Timer0ISR, 1)
{
    ...
    SP_REG = (ray_base_t)OSStack - 1;                                                   //MOV SP, #(OSStack - 1)
    Timer_Reload();                                                                      //LCALL Timer_Reload
    ThreadScan();                                                                        //LCALL ThreadScan
    SP_REG = (ray_base_t)OS_ThreadHandlerIndex[OS_RunningThreadID]->ThreadStackPointer; //MOV SP, TaskSP
    OS_EXIT_CRITICAL();
}
...
void ThreadSwitchTo(ray_thread_t thread)
{
    ...
    SP_REG = (ray_base_t)thread->ThreadStack - 1;
